...
<body>
    <marquee behavior="scroll" direction="left">
        This page is competent to edit transfers   
    </marquee>
    <div class="container">
        <form class="form-container" action="{{ route('transfers.update',$transfer->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <br>
            <label class="number1" for="number">Number of transfer</label>
            <input class="number2" type="number" name="number" value="{{ $transfer->Number}}">
            <br>
            <label class="number1" for="number">The Sender</label>
            <input class="number2" type="text" name="Sender"  value="{{ $transfer->Sender}}" >
            <br>
            <label class="send1" for="send">Send Office</label>
            <select class="send2" name="send" >
                @foreach($offices as $office)
                <option value="{{ $transfer->SendOffice }}">{{ $office->Nameoffice }}</option>
                @endforeach
            </select>
            <br>
            ...
            <input class="number2" type="text" name="Sender"  value="{{ $transfer->Receiver}}" >
            <br>
            <label class="value1" for="value">Transfer Value</label>
            <input class="value2" type="number" name="value" value="{{ $transfer->Value }}">
            <br>
